...
## 2.5 Obligations 抽取：避免 no-op 验收（推荐）

当任务的 `acceptance[]` 只剩“通用约束”时，很容易出现“done 但没有任务独有行为”的假完成。解决方式是把任务文本中隐含的 **obligations（义务清单）** 抽出来，并强制落地为可测试的验收条款。

典型症状：

- Subtask 写了“回合推进/事件发布/失败策略”等义务，但 `acceptance[]` 里没有对应的行为/不变式/失败语义。
- 代码实现了“能跑”，但缺少可审计证据（没有 `Refs:`、没有 `ACC:T<id>.<n>` anchors）。

建议流程（用于 pending / in-progress 优先）：

- 抽取 obligations：`py -3 scripts/sc/llm_extract_task_obligations.py --task-id <id>`
- 参考输入来源：
  - `.taskmaster/tasks/tasks.json`：`description` / `details` / `testStrategy` / `subtasks[]`
  - `.taskmaster/tasks/tasks_back.json`、`.taskmaster/tasks/tasks_gameplay.json`：`acceptance[]` / `test_refs[]`
- 输出证据：`logs/ci/<YYYY-MM-DD>/sc-llm-obligations-task-<id>/verdict.json` 与 `report.md`

门禁判定建议：

- 每条 obligation 必须带 `source_excerpt`（指向原始任务文本片段），否则容易跑偏。
- 若存在 uncovered obligation，则必须 `fail` 并要求补充：
  - `acceptance[]` 的行为/不变式/失败语义条款
  - 每条条款的 `Refs:` 与 `ACC:T<id>.<n>` anchors

对齐策略（止损规则）：

- pending / in-progress：允许结构化补齐（推荐 `llm_align_acceptance_semantics.py --structural-for-not-done`）。
- done：只允许追加（推荐 `llm_align_acceptance_semantics.py --append-only-for-done`），避免“重写历史”导致回归不可追。
