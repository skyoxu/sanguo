...
func before() -> void:
	...
	var loader = preload("res://Game.Godot/Adapters/TestPackIndexInvalidResourceLoader.cs").new()
...
func _on_evt(type, _source, data_json, _id, _spec, _ct, _ts) -> void:
	var evt := str(type)
	_types.append(evt)
	if evt == EVT_START_FAILED:
		_start_failed_payload = str(data_json)

# ACC:T60.3
func test_play_start_fails_when_pack_index_invalid() -> void:
	_types.clear()
	_start_failed_payload = ""
	...
	assert_bool(_start_failed_payload.find("content_pack_unavailable") >= 0).is_true()
