...
func _select_first_alternative_character(char_grid: GridContainer) -> bool:
	for child in char_grid.get_children():
		if child is Button:
			var btn := child as Button
			if btn.visible and not btn.button_pressed:
				btn.emit_signal("pressed")
				return true
	return false

func _start_and_capture_menu_start_payload(menu: Node, nodes: Dictionary) -> Dictionary:
	...
	if not _seen_menu_start:
		...
		if status != null:
			print("T121: menu did not emit ui.menu.start; status='%s'" % status.text)
		assert_bool(false).is_true()
		return {}

	return _parse_json_dict(_last_payload_json)

# acceptance: ACC:T121.1
func test_task121_new_game_setup_presets_are_whitelisted_and_defaults_are_valid() -> void:
...
# acceptance: ACC:T121.2
func test_task121_start_publishes_unique_character_assignments_for_all_players() -> void:
	...
	var payload := await _start_and_capture_menu_start_payload(menu, nodes)
...
# acceptance: ACC:T121.1
func test_task121_commander_open_state_changes_p1_assignment_from_default_selection() -> void:
	var default_menu = preload("res://Game.Godot/Scenes/UI/MainMenu.tscn").instantiate()
	add_child(auto_free(default_menu))
	await get_tree().process_frame
	...
	var open_menu = preload("res://Game.Godot/Scenes/UI/MainMenu.tscn").instantiate()
	add_child(auto_free(open_menu))
	await get_tree().process_frame
	var open_nodes := _find_menu_nodes(open_menu)
	var char_grid: GridContainer = open_nodes["char_grid"]
