...
# acceptance: ACC:T122.1
func test_task122_start_is_refused_when_active_strategem_is_missing() -> void:
...
# acceptance: ACC:T122.1
func test_task122_start_payload_contains_active_and_passive_strategems_when_selected() -> void:
	var menu = preload("res://Game.Godot/Scenes/UI/MainMenu.tscn").instantiate()
	add_child(auto_free(menu))
	await get_tree().process_frame
