...
# acceptance: ACC:T121.3
func test_task121_players_count_is_within_allowed_range_and_carried_in_started_config() -> void:
...
# acceptance: ACC:T121.4
func test_task121_starting_money_preset_is_valid_and_carried_in_started_config() -> void:
	var main = preload("res://Game.Godot/Scenes/Main.tscn").instantiate()
	add_child(auto_free(main))
	await get_tree().process_frame
...
func test_task121_character_assignments_are_complete_and_unique() -> void:
	var main = preload("res://Game.Godot/Scenes/Main.tscn").instantiate()
	add_child(auto_free(main))
	await get_tree().process_frame
