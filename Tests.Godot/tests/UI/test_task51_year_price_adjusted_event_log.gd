...
func test_task51_event_log_includes_year_price_adjusted_details() -> void:
	...
	var summary_label = _translate_summary(EVENT_TYPE_YEAR_PRICE_ADJUSTED)
	assert_str(last).contains(summary_label)
	assert_str(last).not_contains(EVENT_TYPE_YEAR_PRICE_ADJUSTED)
	var multiplier_label = _translate_field(EVENT_TYPE_YEAR_PRICE_ADJUSTED, "detail", "mult.effective_multiplier", "effective_multiplier")
