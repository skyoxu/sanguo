...
class ActiveTaskSidecarTests(unittest.TestCase):
    ...
    def test_build_active_task_payload_should_handle_sparse_single_turn_seq_run_events(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            ...
            out_dir, latest_path = self._build_bundle(
                ...
                summary_payload={
                    ...
                    "reuse_mode": "none",
                    "steps": [{"name": "sc-test", "status": "fail", "rc": 1}],
                },
                execution_context_payload={
                    "schema_version": "1.0.0",
                    "cmd": "sc-review-pipeline",
            ...
            self.assertEqual("", payload["run_event_summary"]["previous_turn_id"])
            self.assertFalse(payload["run_event_summary"]["approval_changed"])
            self.assertIn(f"- Run events latest turn: {run_id}:turn-2 seq=2", markdown)
            self.assertIn("- Run events previous turn: n/a seq=0", markdown)
