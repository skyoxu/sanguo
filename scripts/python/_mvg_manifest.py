...
def validate_manifest(root: Path, doc: dict, executable: bool = False) -> list[str]:
    ...
    try:
        ...
        for test in tests:
            ...
            if 'challenge' in test:
                challenge = test['challenge']
                if test['kind'] != 'gdunit' or not isinstance(challenge, dict):
                    errors.append(f'{label}: challenge must be an object on a gdunit test')
                elif any(not str(challenge.get(key, '')).strip()
                         for key in ('env_var', 'env_value', 'failure_marker')):
                    errors.append(f'{label}: challenge needs env_var, env_value and failure_marker')
        used_tests = set()
        for flow in flows:
            label = flow['id']
            ids = flow['task_ids']
