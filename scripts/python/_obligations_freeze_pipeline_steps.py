...
def build_jitter_batch_command(args: Any, *, raw_path: Path, root: Path) -> list[str]:
    tasks_file = (
        resolve_repo_path(str(getattr(args, "tasks_file", "") or ""), root=root)
        if str(getattr(args, "tasks_file", "") or "").strip()
        else default_tasks_file_path(root)
    )
...
def run_step(step_name: str, cmd: list[str], out_dir: Path, *, root: Path, timeout_sec: int) -> dict[str, Any]:
    ...
    log_path.write_text(
        "\n".join([f"$ {cmd_text}", "", "### stdout", process.stdout or "", "", "### stderr", process.stderr or ""]),
        encoding="utf-8",
    )
    return {
        "name": step_name,
