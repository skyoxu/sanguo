...
SOURCE = 'Game.Core/Domain/SanguoEconomyRules.cs'
MUTANTS = [
    ('reject-min-price-multiplier',
     '=> value >= MinMultiplier && value <= MaxPriceMultiplier && IsHalfStepMultiplier(value);',
     '=> value > MinMultiplier && value <= MaxPriceMultiplier && IsHalfStepMultiplier(value);'),
    ('reject-max-price-multiplier',
     '=> value >= MinMultiplier && value <= MaxPriceMultiplier && IsHalfStepMultiplier(value);',
     '=> value >= MinMultiplier && value < MaxPriceMultiplier && IsHalfStepMultiplier(value);'),
]
TEST = {'id': 'economy-mutation-probes', 'kind': 'dotnet',
        'evidence_level': 'domain-integration',
        'selector': 'Game.Core.Tests.Domain.MvgEconomyMutationProbeTests', 'min_tests': 3}
