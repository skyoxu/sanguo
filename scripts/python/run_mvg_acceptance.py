...
DEFAULT_MANIFEST = 'docs/testing/mvg/sanguo-pilot.json'
...
def register_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--challenge-input', action='store_true',
                        help='After baseline passes, run manifest-declared negative input challenges')


def _run_challenges(doc: dict, execution_root: Path, out: Path, godot_bin: str,
                    deadline: float, base_env: dict, godot_ready: bool) -> list[dict]:
    ...
    for test in targets:
        ...
        env = dict(base_env)
        env[str(challenge['env_var'])] = str(challenge['env_value'])
        result = execute_test(execution_root, test, out / ('challenge-' + test['id']),
                              godot_bin, deadline, env, prewarm=not godot_ready)
        evidence = result['evidence']
        marker = str(challenge['failure_marker'])
        detected = (result['exit_code'] not in {0, 124, 127}
                    and evidence['failed'] > 0
                    and evidence['skipped'] == 0
                    and any(marker in message for message in evidence['failure_messages'])
                    and evidence['reason'] in {'failed-skipped-or-empty-tests', 'reported-suite-failure'})
        result['defect_detected'] = detected
        result['challenge'] = {'env_var': challenge['env_var'],
                               'env_value': challenge['env_value'],
                               'failure_marker': marker}
        results.append(result)
        if not detected:
            raise RuntimeError('Declared input defect was not demonstrated by a test assertion: ' + test['id'])
        godot_ready = True
    return results
    ...
    try:
        ...
        if args.mode != 'run':
            ...
            if args.challenge_input:
                summary['challenges'] = _run_challenges(
                    doc, execution_root, out, args.godot_bin, deadline, env, godot_ready)
